...
passport.serializeUser(function(user, done) { // find what these are doing
  done(null, user.id);
...
app.post("/login",function(req,res,next){ // login handling
  ...
  req.login(user,function(err){
    if(err){console.log(err);} // error while trying to login
      ...
      passport.authenticate('local', function(err, user, info) { // if the local auth method fails it will return 401 error, so to prevent custom error handling methods is needed.
    if (err) {
...
app.post("/signup",function(req,res){
  ...
  const new_user = new User({ // to use passport to authenticate user username and password attr is needed,the object passed in from react also needs username and password attr
...
app.get('/auth/google/Doghotel', // unlike using local auth method this method only doesn't follow user schema, it only create google id attr
